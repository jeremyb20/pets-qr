...
export class ForgotPetsComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private _petService: PetService,private _notificationSvc: NotificationService, private router: Router) {}
    ...
    onForgotPasswordCompanySubmit(){
      ...
      this._petService.forgotPassword(pet).subscribe(data => {
        if(data.success) {
          this.loading = false;
          Swal.fire({
