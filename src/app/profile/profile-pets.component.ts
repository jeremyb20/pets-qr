...
export class ProfilePetsComponent implements OnInit {
  ...
  constructor(private _petService: PetService,private _notificationSvc: NotificationService,private media: MediaService,private formBuilder: FormBuilder ) { 
    ...
    this._petService.getPetDataList(this.pet.id, this.idSecondary,view).subscribe(data => {
     if(data.success){
      ...
      this.newPetInfoForm = this.formBuilder.group({
        ...
        ownerPetName: [this.profile.ownerPetName],
        birthDate: [this.profile.birthDate ],
        phone: [this.profile.phone, [Validators.minLength(8),Validators.pattern(/\d/)]],
        address: [this.profile.address],
        email: [this.profile.email, [Validators.required]],
        age: [this.profile.age, [Validators.minLength(0),Validators.pattern(/\d/)]],
        veterinarianContact: [this.profile.veterinarianContact],
        phoneVeterinarian: [this.profile.phoneVeterinarian, [Validators.minLength(8),Validators.pattern(/\d/)]],
        healthAndRequirements: [this.profile.healthAndRequirements],
  ...
  addNewPetInfoSubmit() {
    ...
    this._petService.updatePetProfile(pet).subscribe(data => {
      if(data.success) {
        this.loading = false;
        Swal.fire({
          position: 'center',
