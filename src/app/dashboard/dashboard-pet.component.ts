...
idEventUpdate: any;
  ...
  constructor(private _petService: PetService, private media: MediaService, private formBuilder: FormBuilder, private ngZone: NgZone, private _location: Location, private _notificationSvc: NotificationService, private router: Router) {
  ...
  stepTrackOrder(step: number){
    this.id = step;
    this._petService.setidTrack(this.id);
  }
  ...
  getPetDataList() {
    ...
    this._petService.getPetDataList(this.pet.id, this.idSecondary, view).subscribe(data => {
      if(data.success){
        ...
        this._petService.setstoreUserData(objectStored);
        this.showInfo = true;
  ...
  getPetProfileList(){
    this._petService.getAllProfileList(this.petPrincipal.id).subscribe(data => {
      this.photoPrincipalPet = data.photo;
      this.seeAllProfile = data.newPetProfile;
    },
    error => {
  ...
  updatePhotoSubmit(){
    this.loading = true;
    this._petService.updatePhotoPetProfile(this.pet.id, this.idSecondary, this.file).subscribe(data => {
      if(data.success) {
  ...
  sendPetStatusLost() {
    ...
    this._petService.sendNewPetStatusEvent(form, status).subscribe(data => {
      if(data.success) {
  ...
  updateReportStatus() {
    ...
    this._petService.updatePetStatusReport(status).subscribe(data => {
      if(data.success) {
        $('#reportProfileModal').modal('hide');
        this._notificationSvc.success('Hola '+this.pet.petName+'', data.msg, 6000);
        this.loading = false;
  ...
  onSubmit() {
    ...
    if(this.i.genderSelected.value  === 'Genero del Can'){
      ...
      if(this.markersNewPet.length != 0) {
        ...
        this._petService.registerNewPetByUserPet(newPet,this.file).subscribe(data => {
          if(data.success) {
            this.loading = false;
